...
singletonexon::singletonexon(char feature, int start, int end) {
    if (feature == 'C') {
        _feature = CDS;
    } else if (feature == 'U') {
        _feature = UTR;
//                } else if (feature == 'x') {
    ...
    //cerr << feature << " => " << _feature << endl;
    _start = start;
    _end = end;
}

singletonexon::singletonexon(Feature feature, int start, int end) {
    _feature = feature;
    _start = start;
    _end = end;
}

singletonexon::singletonexon(const singletonexon& rhs) {
    _feature = rhs._feature;
    _start = rhs._start;
    _end = rhs._end;
}

const singletonexon& singletonexon::operator = (const singletonexon& rhs) {
...
string singletonexon::feature() const { 
    switch (_feature) {
    case CDS: return "C";
    case UTR: return "U";
...
featuredexon::featuredexon(const exon& rhs) {
    _num_exons = 1;
    _borders = new int[2];
    _features = rhs.feature();
    _borders[0] = rhs.start();
    _borders[1] = rhs.end();
    _features = rhs.feature();
}

featuredexon::featuredexon(const featuredexon& rhs) {
    _num_exons = rhs._num_exons;
    _borders = new int[_num_exons + 1];
    _features = rhs._features;
    memcpy(_borders, rhs._borders, sizeof(int) * (_num_exons + 1));
}

featuredexon& featuredexon::operator = (const featuredexon& rhs) {
    if (this == &rhs) return *this;
    _num_exons = rhs._num_exons;
    _borders = new int[_num_exons + 1];
    _features = rhs._features;
...
featuredexon::~featuredexon() {
    delete[] _borders;
}                

void featuredexon::add(const exon& e) throw (invalid_argument) {
    if (e.start() == this->end() + 1) {
        int n = _num_exons + 1;
        int* b = new int[n+1];
        memcpy(b, _borders, sizeof(int) * n);
        b[n] = e.end();
        _num_exons = n;
        delete[] _borders;
        _borders = b;
        _features = _features + e.feature();
    } else if (e.end() + 1 == this->start() + 1) {
        int n = _num_exons + 1;
        int* b = new int[n+1];
        b[0] = e.start();
        memcpy(b + 1, _borders, sizeof(int) * n);
        _features = e.feature() + _features;
        _num_exons = n;
        delete[] _borders;
        _borders = b;
    } else {
        throw invalid_argument("two exons are not proximal");
    }
}

target_probe::target_probe(const char* chromosome, int position) {
    _position = position;
    _chromosome = tktools::bio::convert_chromosome_to_code(chromosome);
}

target_probe::target_probe(int chromosome, int position) {
    _position = position;
    _chromosome = chromosome;
}

string target_probe::chromosome() const {
    return convert_code_to_chromosome(_chromosome);
}

twocolor_probe::twocolor_probe(const string& name, const char* chromosome, int position) 
  : target_probe(chromosome, position) 
{
    _id = name;
    _values[0] = _values[1] = _values[2] = _values[3] = NO_DATA;
}

twocolor_probe::twocolor_probe(const string& name, int chromosome, int position) 
  : target_probe(chromosome, position) {
    _id = name;
    _values[0] = _values[1] = _values[2] = _values[3] = NO_DATA;
}
...
void twocolor_probe::output(ostream& ost) const {
    if (_values[0] == NO_DATA) {
        //
    } else {
        double logratio = log(_values[0] / _values[1]) * _inv_log_2;
        ost << ".\t"<< logratio;
    }
}
...
void seq_gene::add_exon(exon* exon) {
    _exons.push_back(exon);
    //cerr << __func__ << " : " << _exons.size() << endl;
}

void seq_gene::add_exon(exon::Feature feature, int start, int end) {
    _exons.push_back(new singletonexon(feature, start, end));
}
...
void seq_gene::set_location(int chrm, char orientation, int start, int end) {
    ...
    _start = start;
    _end = end;
    if (end - start + 1 > maximum_size) {
        maximum_size = end - start + 1;
    }
}
...
void seq_gene::serialize(ostream& ost) const {
    ...
    if (_exons.size() > 0) {
        ...
        for (int i = 0; i < (int)_exons.size(); i++) {
            ...
            const exon* e = _exons[i];
            const featuredexon* fe = dynamic_cast<const featuredexon*>(e);
            string features = e->feature();
            if (fe != NULL) {
                for (int j = 0; j < fe->count(); j++) {
                    ost << features.c_str()[j] << "[" << (fe->border(j) + (j > 0 ? 1 : 0)) << ":" << fe->border(j+1) << "]";
                }
            } else {
                ost << features << "[" << e->start() << ":" << e->end() << "]";
            }
            
            //ost << _exons[i]->feature() << ":" << _exons[i]->start() << ":" << _exons[i]->end();
            // char feature;
    ...
    // ost << ">GeneID:" << _geneid << "\t" << _symbol << "\t" << _id;
    // ost << "\tchr" << convert_code_to_chromosome(_chrm) << ":" << _orientation << ":" << _start << ":" << _end << "\n";
    // for (int i = 0; i < (int)probes.size(); i++) {
    //     ost << probes[i]->id() << " " << probes[i]->chromosome() << ":" << probes[i]->position() << "\t";
    //     int pos = probes[i]->position();
    //     if (_orientation == '+') {
    //         cout << pos - _start << "\t";
    //     } else {
    //         cout << _end - pos << "\t";
...
namespace {
    const char* cache_file = ".seq_gene.sp";
                        ...
                        for (int i = 0; i < (int)exons.size(); i++) {
                            ...
                            for (;;) {
                                ...
                                if (c == '\0') {
                                    ...
                                    //     f = exon::OTHERS;
                                    // }
                                    exon* e_ = new singletonexon(*p0, atoi(p1), atoi(p2));
                                    if (e == NULL) {
                                        e = e_;//new singletonexon(*p0, atoi(p1), atoi(p2));
                                    } else {
                                        featuredexon* fe = dynamic_cast<featuredexon*>(e);
                                        if (fe == NULL) {
                                            fe = new featuredexon(*e);
                                            delete e;
                                        }
                                        fe->add(*e_);
                                        delete e_;
                            ...
                            //         feature = exon::OTHERS;
                            //     } else {
                            //         throw invalid_argument(string("unknown exon data ") + exons[i]);
                            //     }
...
//    map<string,pair<int,string> > rna2gene;
    ...
    set<int> pseudo;
    int num_lines = 0;
    map<string,vector<singletonexon*> > exons;
    map<string,string> contig_assignment;
    while (!file_in.eof()) {
        string line;
        getline(file_in, line);
        num_lines++;
...
//                cerr << i << " : " << items[i] << endl;
            ...
            if (items[col_type] == "GENE") {
                ...
                map<string,string>::const_iterator bit = contig_assignment.find(refseq);
                if (bit == contig_assignment.end()) {
                    contig_assignment[refseq] = items[col_contig];
                } else {
                    if (bit->second != items[col_contig]) {
                        //cerr << "skip " << items[col_contig] << " // " << bit->second << endl;
                        continue;
                    }
                }
                if (items[col_type] == "UTR" || items[col_type] == "CDS") {
                    //if (refseq == "NM_001012623.1") {
                    //cerr << contig_assignment[refseq] << ", " << items[col_contig] << endl;
                    //}
                    //if (bac_assignment.find(refseq) != bac_assignment.end() && bac_assignment[refseq] != items[col_bac]) {
...
//                    cout << refseq << " // " << items[col_type] << ":" << es << "-" << ee << endl;
                    ...
                    map<string,vector<singletonexon*> >::iterator eit = exons.find(refseq);
                    exon::Feature feature = items[col_type] == "UTR" ? exon::UTR : exon::CDS;
                    if (eit == exons.end()) {
                        vector<singletonexon*> exons_for_rna;
                        exons_for_rna.push_back(new singletonexon(feature, es, ee));
                        exons[refseq] = exons_for_rna;
                    } else {
                        eit->second.push_back(new singletonexon(feature, es, ee));
                    }
    ...
    for (vector<seq_gene*>::iterator it = genes.begin(); it != genes.end(); it++) {
        ...
        map<string,vector<singletonexon*> >::const_iterator sit = exons.find(g->_id);
        if (sit != exons.end()) {
            vector<exon*> bound;
            vector<singletonexon*> exons = sit->second;
            //cerr << g->_symbol << "\t" << g->_id << "\t" << exons.size() << "\t";
            sort(exons.begin(), exons.end(), exon::compare_exon_position);
            //exon* e_ = NULL;

            vector<exon*> aligned;
            for (int i = 0; i < (int)exons.size(); i++) {
                const singletonexon* e = exons[i];
                if (i < (int)exons.size() - 1 && exons[i + 1]->start() != e->end() + 1) {
                    aligned.push_back(new singletonexon(*e));
                } else {
                    featuredexon* fe = new featuredexon(*e);
                    int j;
                    for (j = i + 1; j < (int)exons.size(); j++) {
                        if (fe->end() + 1 == exons[j]->start()) {
                            fe->add(*exons[j]);
                        } else {
                            break;
                        }
                    }
                    aligned.push_back(fe);
                    i = j;
                }
            }
            g->_exons = aligned;
            //cerr << aligned.size() << endl;
            for (vector<singletonexon*>::const_iterator it = sit->second.begin(); 
            ...
            //     if (e_ != NULL) {
            //         cerr << "_ new exon_\n";
            //         if (e_->end() + 1 == exons[i]->start()) {
            //             cerr << "_ should be bound\n";
            //             featuredexon* fe = dynamic_cast<featuredexon*>(e_);
            //             if (fe == NULL) {
            //                 cerr << "_ prev is not featured\n";
            //                 fe = new featuredexon(*e_);
            //                 delete e_;
            //             }
            //             fe->add(*exons[i]);
            //             //delete exons[i];
            ...
            // }
            // if (e_ != NULL) {
            //     cerr << "_ terminal exon\n";
            //     g->add_exon(e_);
            // }
            // cout << "\t" << g->count_exons() << endl;

            //for (vector<singletonexon*>::const_iterator eit = sit->second.begin(); eit != sit->second.end(); eit++) {
...
//     if (_orientation == '+') {
//         for (int i = 0; i < (int)_exons.size(); i++) {
//             if (i == 0 || _exons[i]->start() - _exons[i-1]->end() > 1) {
//                 index++;
//             }
//         }
//     } else {
//         for (int i = (int)_exons.size() - 1; i >= 0; i--) {
//             if (i == (int)_exons.size() - 1 || _exons[i]->start() - _exons[i-1]->end() > 1) {
//                 index++;
//             }
//         }
//     }
...
int seq_gene::get_exon_index(int position) const {
    int left = 0;
    int right = (int)_exons.size();
    for (;;) {
        int center = (left + right) / 2;
        const exon* e = _exons[center];
        if (left == center) {
            if (e->start() <= position && position <= e->end()) {
                return center;
            } else {
                return -1;
            }
